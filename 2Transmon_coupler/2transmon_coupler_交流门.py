...
EJ_q1=24*2*np.pi
EJ_q2=24*2*np.pi
g_qc1=300e-3*2*np.pi
g_qc2=300e-3*2*np.pi
# phi_e=0.5
# phi_e=0.5
# phi_e0=0
...
omega02_dr=np.linspace(omega02_d*2.01,omega02_d*2.03,6)
print(phi_e0)

def phi_et(t,omega_d):
    if t<10:
        phi_e=A*(1-np.cos(2*np.pi*t/20))*0.5*np.sin(omega_d*2*np.pi*t)+phi_e0
    elif t>40:
        phi_e=A*(1-np.cos(2*np.pi*(t-30)/20))*0.5*np.sin(omega_d*2*np.pi*t)+phi_e0
    else:
...
def cos_phie1(t,args):
    ...
    phi_e=phi_et(t,omega_d)
    return(np.cos(C_12*C_c2/C_t**2*phi_e*2*np.pi))
plt.figure()
plt.plot(time_r,[cos_phie1(i,args) for i in time_r])
plt.plot(time_r,[sin_phie1(i,args) for i in time_r])
